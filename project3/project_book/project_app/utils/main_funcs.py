from project_app.services.embedding_api import en
...
def predict_book(compare_lists, mylist):
    
    embeddings = []
    labels = []

    for i in range(0,len(compare_lists)):
        if compare_lists[i].isbn== mylist.isbn:
            continue
        embeddings.append(compare_lists[i].embedding)
        labels.append(compare_lists[i].isbn)

    classifier = LogisticRegression()
    classifier.fit(embeddings, labels)

    predict_embedding = en.encode(texts=[mylist.title+mylist.author+mylist.description])
    prediction = classifier.predict(predict_embedding)
